...
export const AdminDashboard = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // fetch unread count
        const { getUnreadCount } = await import('../../api/messages.js');
  ...
  // Join admin private socket to receive new private messages and update unread
  const authUser = useSelector((state) => state.auth.user);

  useEffect(() => {
    if (!users || users.length === 0 || !authUser) return;
    (async () => {
      const { getUnreadCount } = await import('../../api/messages.js');
      try {
        const uid = authUser._id;
        if (!uid) return;
        // join socket private room
        socket.emit('joinPrivate', uid);

        const onPrivate = ({ message }) => {
          // increment only if current admin is the recipient
          if (String(message.recipient._id) === String(uid)) {
            setUnread((u) => u + 1);
          }
        };
        ...
        // periodically sync unread count
