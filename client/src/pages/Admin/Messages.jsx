...
export default function AdminMessages() {
  ...
  // Robust current user id/name resolution (supports nested payload shapes)
  ...
  useEffect(() => {
    ...
    socket.on('newMessage', (message) => {
      ...
      if (String(currentRoom) === String(message.roomId)) {
        setMessages((prev) => {
          const optIndex = prev.findIndex(m => m._optimistic && String(m.userId) === String(message.userId) && m.text === message.text && Math.abs(new Date(m.timestamp) - new Date(message.timestamp)) < 5000);
          if (optIndex !== -1) {
            const copy = [...prev];
  ...
  const handleJoinRoom = async (roomId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Normalize message userId to string for stable comparison with current user id
                ...
                {loadingMessages ? (
                  ...
                  messages.map((msg) => {
                    const key = msg.id || msg._id || `${msg.roomId}-${msg.timestamp || msg.createdAt}-${Math.random()}`;
                      ...
                      console.log(`📨 Admin page msg render: msgUserId='${normalizedMsgUserId}', user._id='${normalizedCurrentUserId}', isMine=${isMine}, userName='${msg.userName}'`);
                    ...
                    return (
                      ...
                      >
                        {msg.isSystem ? (
                          ...
                          <div
                            className={`max-w-xs px-4 py-2 rounded-lg ${isMine ? 'bg-blue-600 text-white rounded-br-none ml-auto' : 'bg-white text-gray-900 border border-gray-200 rounded-bl-none mr-auto'}`}
                          >
