...
export const CreateAuction = () => {
  ...
  // Helpers: format local date-time for <input type="datetime-local" /> (YYYY-MM-DDTHH:MM)
  const formatToLocalInput = (d) => {
  ...
  // min allowed start: now (local)
  const todayLocal = formatToLocalInput(new Date());

  // max start date: now + 15 days
  const maxStart = new Date();
  maxStart.setDate(maxStart.getDate() + 15);
  const maxStartDateLocal = formatToLocalInput(maxStart);

  // max end date: start + 15 days (if start provided)
  let maxEndDateLocal = "";
  if (formData.itemStartDate) {
    const end = new Date(formData.itemStartDate);
    end.setDate(end.getDate() + 15);
    maxEndDateLocal = formatToLocalInput(end);
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    min={formData.itemStartDate || todayLocal}
                    max={maxEndDateLocal}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    required
                  />
